...
class Field
{
    ...
    createElement( num, event, dt )
    {
        ...
        let element = document.createElement( "div" );
            ...
            {
                element.dataset.xy = `${this.COORDINATES.getX()};${this.COORDINATES.getY()}`;
            }
    ...
    static findBgElementByCoordinates( _coord, _container )
    {
        let Node;

        _container.childNodes[1].childNodes.forEach( node => {
            if( node.dataset.xy.split(";")[0] == _coord.getX() && node.dataset.xy.split(";")[1] == _coord.getY() )
            {
                //console.log(node);
                Node = node;
            }
