...
const fieldAction = event => {
    ...
    switch ( event.type )
    {
        ...
        /*GAME.recursiveDiscover(element).forEach(item => {
            let el = Field.findElementByCoordinates( new Coordinates(parseInt( item.dataset.xy.split(";")[0] ), parseInt( item.dataset.xy.split(";")[1] )), MENU.FieldMap );
